...
def upload_image(image_path, source_name, label, vFlag):
    ...
    post_database(unique_id, source_name, label, vFlag)
...
def post_database(image_id, source_name, label, vFlag):
    """
    ...
    images_table = dynamodb.Table(global_table)
...
def random_image():
    ...
    # access the dynamodb
    dynamodb = boto3.resource('dynamodb', region_name=dynamodb_region, endpoint_url=dynamodb_endpoint)
    table = dynamodb.Table(global_table)
